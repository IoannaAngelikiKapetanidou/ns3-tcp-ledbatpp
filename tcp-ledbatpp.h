...
class TcpLedbatpp : public TcpNewReno
{
private:
  /**
...
public:
  ...
  void Slowdown (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked);
...
private:
  ...
  bool init_slowstart;               //!< Flag for the initial slowstart
  double slowdown_init;                            
  double slowdown_end;                            
  double slowdown_period;
  bool init_slowdown;                //!< Flag for the initial slowdown
  bool slowdown_init_flag;
  bool slowdown_end_flag;
  int constant;                     //constant used in cwnd increase when delay > target delay
  double cwnd_freeze;
  bool frozen_cwnd;
};
