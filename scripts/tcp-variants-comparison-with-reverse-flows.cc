...
TraceCwnd(std::string cwnd_tr_file_name, uint32_t nodeId)
{
    ...
    Config::Connect("/NodeList/" + std::to_string(nodeId) +
                        "/$ns3::TcpL4Protocol/SocketList/1/CongestionWindow",
...
TraceSsThresh(std::string ssthresh_tr_file_name, uint32_t nodeId)
{
    ...
    Config::Connect("/NodeList/" + std::to_string(nodeId) +
                        "/$ns3::TcpL4Protocol/SocketList/1/SlowStartThreshold",
...
TraceRtt(std::string rtt_tr_file_name, uint32_t nodeId)
{
    ...
    Config::Connect("/NodeList/" + std::to_string(nodeId) + "/$ns3::TcpL4Protocol/SocketList/1/RTT",
...
TraceRto(std::string rto_tr_file_name, uint32_t nodeId)
{
    ...
    Config::Connect("/NodeList/" + std::to_string(nodeId) + "/$ns3::TcpL4Protocol/SocketList/1/RTO",
...
TraceNextTx(std::string& next_tx_seq_file_name, uint32_t nodeId)
{
    ...
    Config::Connect("/NodeList/" + std::to_string(nodeId) +
                        "/$ns3::TcpL4Protocol/SocketList/1/NextTxSequence",
...
TraceInFlight(std::string& in_flight_file_name, uint32_t nodeId)
{
    ...
    Config::Connect("/NodeList/" + std::to_string(nodeId) +
                        "/$ns3::TcpL4Protocol/SocketList/1/BytesInFlight",
...
InitializeCounters()
{
    for (std::size_t i = 0; i < num_flows; i++)
    {
     ...
     for (std::size_t i = 0; i < num_reverse_flows; i++)
    {
        rxS2R2Bytes[i] = 0;
    }
...
PrintThroughput(Time measurementWindow)
{
     ...
     for (std::size_t i = 0; i < num_active_flows; i++)
    {
        rxS2R2Throughput << Simulator::Now().GetSeconds() << "s " << i << " "
                         << (rxS2R2Bytes[i] * 8) / (Simulator::Now ().GetSeconds()) / 1e6
                         << std::endl;
    }
...
PrintFairness(Time measurementWindow)
{
    ...
    average = ((sum / num_flows) * 8 / Simulator::Now ().GetSeconds()) / 1e6;
    fairness = static_cast<double>(sum * sum) / (num_flows * sumSquares);
    ...
    fairnessIndex << Simulator::Now ().GetSeconds () << " Average throughput for S2-R2 flows: " << std::fixed << std::setprecision(2)
                  << average << " Mbps; fairness: " << std::fixed << std::setprecision(3)
                  << fairness << std::endl;
   ...
   // fairness = static_cast<double>((sum1 + sum2) * (sum1 + sum2))/ ((num_active_flows + num_flows) * (sumSquares1 +  sumSquares2));
   //fairness = static_cast<double>((sum1 * sum1 + sum2 * sum2)) / ((num_flows * sumSquares1) + (num_active_flows * sumSquares2));
    fairnessIndex << Simulator::Now ().GetSeconds () << " Aggregate user-level throughput for flows through bottleneck link: "
                  << static_cast<double>(sum * 8) / 1e9 << " Gbps; fairness: " << std::fixed << std::setprecision(3)
                  << fairness << std::endl;
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("num_reverse_flows", "Number of reverse flows", num_reverse_flows);
  ...
  /*  if (transport_prot == "ns3::TcpWestwoodPlus")
    ...
    rxS2R2Bytes.reserve(num_reverse_flows);
    ...
    sources.Create(num_flows);
     //reverse traffic
    NodeContainer reverse_sources;
    reverse_sources.Create (num_reverse_flows);
   
    NodeContainer sinks;
    sinks.Create(num_flows);   
  
    NodeContainer reverse_sinks;
    reverse_sinks.Create (num_reverse_flows);
   ...
   AccessLink.SetChannelAttribute ("Delay", StringValue (access_delay));
    ...
    Ipv4InterfaceContainer reverse_sink_interfaces;
    ...
    for (uint32_t i = 0; i < num_flows; i++)
    {
     ...
     for (int i = 0; i < num_reverse_flows ; i++)
    
    { NetDeviceContainer devices;
      devices = AccessLink.Install (reverse_sources.Get (i), gateways.Get (1));
      tchPfifo.Install (devices);
      address.NewNetwork ();
      Ipv4InterfaceContainer interfaces = address.Assign (devices);
      
      devices = LocalLink.Install (gateways.Get (0), reverse_sinks.Get (i));
      if (queue_disc_type.compare ("ns3::PfifoFastQueueDisc") == 0)
        {
          tchPfifo.Install (devices);
        }
      ...
      address.NewNetwork ();
      interfaces = address.Assign (devices);
      reverse_sink_interfaces.Add (interfaces.Get (1));
   ...
   /*   devices = BottleneckLink.Install (gateways.Get (0), gateways.Get (1));
      ...
      reverse_address.NewNetwork ();
      interfaces = reverse_address.Assign (devices);*/
    }
    ...
    std::vector<Ptr<PacketSink>> s2r2Sinks;
    s2r2Sinks.reserve(num_reverse_flows);
    ...
    for (uint32_t i = 0; i < sources.GetN(); i++)
    {
    	...
    	TypeId tid;
    	if (transport_prot == "ns3::TcpWestwoodPlus"){
    		tid = TcpWestwood::GetTypeId();
    		}
    	else tid = TypeId::LookupByName(transport_prot);
    	
	std::stringstream nodeId;
	nodeId << sources.Get(i)->GetId();
	std::string specificNode = "/NodeList/" + nodeId.str() + "/$ns3::TcpL4Protocol/SocketType";
	Config::Set(specificNode, TypeIdValue(tid));
	
	if (transport_prot == "ns3::TcpWestwoodPlus"){
	
		specificNode = "/NodeList/" + nodeId.str() + "/$ns3::TcpSocketBase/TcpWestwood::ProtocolType";
		Config::Set(specificNode, EnumValue(TcpWestwood::WESTWOODPLUS));
	}
    	
    	Ptr<Socket> SourceSocket = Socket::CreateSocket(sources.Get(i), TcpSocketFactory::GetTypeId());
    
    	nodeId << sinks.Get(i)->GetId();
	specificNode = "/NodeList/" + nodeId.str() + "/$ns3::TcpL4Protocol/SocketType";
	Config::Set(specificNode, TypeIdValue(tid));
        Ptr<Socket> SinkSocket = Socket::CreateSocket(sinks.Get(i), TcpSocketFactory::GetTypeId());
     ...
     for (uint32_t i = 0; i < reverse_sources.GetN(); i++)
    {
    	//sockets
    	TypeId tid = TypeId::LookupByName("ns3::TcpLedbatpp");
	std::stringstream nodeId;
	nodeId << reverse_sources.Get(i)->GetId();
	std::string specificNode = "/NodeList/" + nodeId.str() + "/$ns3::TcpL4Protocol/SocketType";
	Config::Set(specificNode, TypeIdValue(tid));
    	
    	Ptr<Socket> reverseSourceSocket = Socket::CreateSocket(reverse_sources.Get(i), TcpSocketFactory::GetTypeId());
    
    	nodeId << reverse_sinks.Get(i)->GetId();
	specificNode = "/NodeList/" + nodeId.str() + "/$ns3::TcpL4Protocol/SocketType";
	Config::Set(specificNode, TypeIdValue(tid));
        Ptr<Socket> reverseSinkSocket = Socket::CreateSocket(reverse_sinks.Get(i), TcpSocketFactory::GetTypeId());
    	
        AddressValue remoteAddress(InetSocketAddress(reverse_sink_interfaces.GetAddress(i, 0), port));
        Config::SetDefault("ns3::TcpSocket::SegmentSize", UintegerValue(tcp_adu_size));
       // BulkSendHelper ftp("ns3::TcpSocketFactory", Address());
       ...
       ftp.SetAttribute("OnTime", StringValue ("ns3::ConstantRandomVariable[Constant=75]"));
        ...
        ApplicationContainer sourceApp = ftp.Install(reverse_sources.Get(i));
        sourceApp.Start(Seconds(start_time + i*interval));
        sourceApp.Stop(Seconds(stop_time - 1));        
        

        sinkHelper.SetAttribute("Protocol", TypeIdValue(TcpSocketFactory::GetTypeId()));
        ApplicationContainer sinkApp = sinkHelper.Install(reverse_sinks.Get(i));
       
        Ptr<PacketSink> packetSink = sinkApp.Get(0)->GetObject<PacketSink>();         
        s2r2Sinks.push_back(packetSink);
    ...
    rxS2R2Throughput.open("tcp-example-s2-r2-throughput.dat", std::ios::out);
    rxS2R2Throughput << "#Time(s) flow throuput(Mb/s)" << std::endl;
    ...
    for (std::size_t i = 0; i < num_reverse_flows; i++)
    {
        s2r2Sinks[i]->TraceConnectWithoutContext("Rx", MakeBoundCallback(&TraceS2R2Sink, i));
    }
		...
		//                        &TraceNextRx,
		   //                     prefix_file_name + flowString + "-next-rx.data",
		     ...
		     //enabled only by granually-joining flows    
		  /*Simulator::Schedule(Seconds(start_time * index + measurementWindow.GetSeconds()),
		                &PrintThroughput,
		                measurementWindow);
	    	    Simulator::Schedule(Seconds(start_time * index + measurementWindow.GetSeconds()),
		                &PrintFairness,
		...
		//                        &TraceNextRx,
		   //                     prefix_file_name + flowString + "-next-rx.data",
		     //                   num_flows + index + gateways.GetN());
      		    ...
      		    Simulator::Schedule(Seconds(start_time + index*interval),&CountActiveFlows);      
