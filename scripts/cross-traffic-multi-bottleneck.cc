...
main(int argc, char* argv[])
{
    ...
    double error_p = 0.0;
    uint16_t num_gateways=4; //works for even numbers
  ...
  /*  if (transport_prot == "ns3::TcpWestwoodPlus")
   ...
   for (uint32_t g = 0; g < num_gateways; g=g+2){ 
        NetDeviceContainer devices;
        devices = AccessLink.Install(sources.Get(f), gateways.Get(g));
        tchPfifo.Install(devices);
        address.NewNetwork();
        Ipv4InterfaceContainer interfaces = address.Assign(devices);
	
        devices = LocalLink.Install(gateways.Get(g+1), sinks.Get(f));
      ...
      sink_interfaces.Add (interfaces.Get (1));
        ...
        devices = BottleneckLink.Install (gateways.Get (g), gateways.Get (g+1));
      ...
      devices = AccessLink.Install (reverse_sources.Get (rf), gateways.Get (g+1));
      tchPfifo.Install (devices);
      address.NewNetwork ();
      interfaces = address.Assign (devices);
      
      devices = LocalLink.Install (gateways.Get (g), reverse_sinks.Get (rf));
      if (queue_disc_type.compare ("ns3::PfifoFastQueueDisc") == 0)
        {
          tchPfifo.Install (devices);
        }
   ...
   /*   devices = BottleneckLink.Install (gateways.Get (0), gateways.Get (1));
      ...
      interfaces = reverse_address.Assign (devices);*/
       ...
       if ( (g+2) < num_gateways ) {
       	devices = BottleneckLink.Install (gateways.Get (g+1), gateways.Get (g+2));
       	address.NewNetwork();
        	interfaces = address.Assign(devices);
       	}
        else{
       	devices = AccessLink.Install(sources.Get(f), gateways.Get(0));
        tchPfifo.Install(devices);
        address.NewNetwork();
        Ipv4InterfaceContainer interfaces = address.Assign(devices);
	
        devices = LocalLink.Install(gateways.Get(num_gateways-1), sinks.Get(f));
        if (queue_disc_type == "ns3::PfifoFastQueueDisc")
        {
            tchPfifo.Install(devices);
        }
    ...
    for (uint32_t i = 0; i < sources.GetN(); i++)
    {
		...
		tid = TypeId::LookupByName("ns3::TcpLedbatpp");
